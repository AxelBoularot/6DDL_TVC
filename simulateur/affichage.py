...
def graphiques(hist, evts):
    ...
    axs[0, 0].axvline(C.burn_time_end, color='r', ls='--', label='Burnout')
    if evts.t_chute:
        axs[0, 0].axvline(evts.t_chute, color='c', ls=':', label='Parachute')
    axs[0, 0].set_title('Altitude'); axs[0, 0].set_ylabel('m'); axs[0, 0].legend()

    axs[0, 1].plot(tL, hist['ang_x'], color='purple', lw=1.8, label='X-Z plane (pitch)')
    axs[0, 1].plot(tL, hist['ang_y'], color='teal', lw=1.8, label='Y-Z plane (yaw)')
    axs[0, 1].axhline(0, color='k', lw=0.8, ls=':')
    axs[0, 1].axvline(C.burn_time_end, color='r', ls='--')
    axs[0, 1].set_title('Rocket angle from vertical'); axs[0, 1].set_ylabel('deg')
    axs[0, 1].legend()

    axs[0, 2].plot(tL, hist['dp'], color='purple', lw=1.3, label='Nozzle pitch')
    axs[0, 2].plot(tL, hist['dy'], color='teal', lw=1.3, label='Nozzle yaw')
    axs[0, 2].axvline(C.burn_time_end, color='r', ls='--')
    axs[0, 2].set_title('Actual nozzle angle (delay + backlash)'); axs[0, 2].set_ylabel('deg')
    axs[0, 2].legend()

    axs[1, 0].plot(tL, hist['mass'], 'k-', lw=2)
    axs[1, 0].axvline(C.burn_time_end, color='r', ls='--')
    axs[1, 0].set_title('Total mass'); axs[1, 0].set_ylabel('kg')
    ...
    axs[1, 1].set_title('CG & CP from nose tip'); axs[1, 1].set_ylabel('m')
    axs[1, 1].invert_yaxis(); axs[1, 1].legend()

    axs[1, 2].plot(tL, hist['Ir'], 'purple', lw=2)
    axs[1, 2].axvline(C.burn_time_end, color='r', ls='--')
    axs[1, 2].set_title('Transverse moment of inertia'); axs[1, 2].set_ylabel('kg.m2')

    for a in axs.ravel():
        a.grid(alpha=0.3)
    for a in axs[1]:
        a.set_xlabel('t (s)')
    plt.suptitle('6-DOF TVC simulation: variable mass/CG/inertia, motor ejection', fontsize=14)
    plt.tight_layout()
    return fig
